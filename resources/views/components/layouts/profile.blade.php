...
<body class="bg-gray-50 text-gray-800 antialiased font-sans">
    ...
    <div class="flex h-screen overflow-hidden bg-gray-50">
        ...
        <div id="sidebarOverlay" onclick="toggleSidebar()"
            class="fixed inset-0 z-20 bg-black/50 hidden lg:hidden backdrop-blur-sm transition-opacity"></div>

        <aside id="sidebar"
            class="fixed inset-y-0 left-0 z-30 w-64 bg-dark-900 text-white transform -translate-x-full lg:translate-x-0 lg:static lg:inset-auto transition-transform duration-300 ease-in-out flex flex-col h-full shadow-2xl shrink-0">
            ...
            <div class="h-20 flex items-center px-8 border-b border-gray-800/50">
                <a href="/" class="text-xl font-bold tracking-wide flex items-center gap-3">
                    <div
                        class="w-9 h-9 rounded-xl bg-primary-500 flex items-center justify-center text-white shadow-lg shadow-primary-500/20">
            ...
            <nav class="flex-1 px-4 py-6 space-y-2 overflow-y-auto no-scrollbar">
                <a href="{{ route('customer.profile') }}"
                    class="{{ request()->routeIs('customer.profile') ? 'bg-primary-600 text-white shadow-lg shadow-primary-500/20' : 'text-gray-400 hover:bg-gray-800 hover:text-white' }} flex items-center px-4 py-3 rounded-xl transition-all group">
            ...
            <div class="p-4 border-t border-gray-800/50">
                <form method="POST" action="{{ route('logout') }}">
                    @csrf
                    <button type="submit"
                        class="flex w-full items-center px-4 py-3 text-red-400 hover:bg-red-500/10 hover:text-red-500 rounded-xl transition-all group">
                        <i
                            class="fa-solid fa-arrow-right-from-bracket w-6 text-lg group-hover:translate-x-1 transition-transform"></i>
        ...
        <div class="flex-1 flex flex-col min-w-0 overflow-hidden">
            ...
            <header
                class="bg-white border-b border-gray-200 lg:hidden flex items-center justify-between px-4 h-16 shrink-0 z-10">
                <button onclick="toggleSidebar()"
                    class="text-gray-600 focus:outline-none p-2 rounded-md hover:bg-gray-100">
